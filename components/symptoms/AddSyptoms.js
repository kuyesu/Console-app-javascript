...
;
  return <Center w="100%">
      <Box maxW="300" w="100%">
        <Heading mb="2" size="md" color={COLORS.primary}>
          {date.format(now, pattern)}
        </Heading>
        <VStack space={4}>
          <HStack space={2}>
            <Input flex={1} onChangeText={v => setInputValue(v)} value={inputValue} placeholder="Add Symptom" />
            <IconButton borderRadius="sm" variant="solid" icon={<Icon as={Feather} name="plus" size="sm" color={COLORS.darkgreen} />} onPress={() => {
            addItem(inputValue);
            setInputValue("");
          }} />
          </HStack>
